...
export function routeToCategory(category) {
  ...
  if (category.parent == 0) {
    ...
    const parentCategorySlug =
      store.state.productsCategories.items?.[category.parent].slug;
    routerObject = router.push({
...
/**
 * Отделяет pathname, готовит к вью-роутеру
 *
 * (?) - мона его раписать это для именованных маршрутов.
 *      Но это когда будет понимание и необходимость использования вложенных маршрутов
 */
export function getPathByURL(url, returnType = "string") {
  let startPos = url.indexOf(siteURL());
  if (startPos !== 0)
    throw `Адрес сайта в аргументе: ${url} и фактический адрес сайта: ${siteURL()} не совпадают`;
  let pathName = url.slice(siteURL().length); // exm. 'orders/'
  switch (returnType) {
    case "string":
      return `/${pathName}`;
    case "array":
